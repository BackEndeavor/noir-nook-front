<script lang="ts">
    ...
    import {deletePost, summaryPost, updatePost} from "../../../../backend";
    ...
    function savePost() {
        if (!data || !data.post)
            return;
        updatePost(data.session.access_token, data.post);
        toast.addInfo('Post successfully updated!', {newToast: {duration_ms: 2500}});
    }
...
<Post>
    ...
    <PostBody>
        {#if data.post && data.post.ok}
            <PostTitle>
                <input type="text" bind:value={data.post.title} class="input input-bordered w-full max-w-xs"/>
            </PostTitle>
            <PostMetadata>
                <Avatar>
            ...
            <MarkdownEditor mode="tabs" theme="github" bind:value={data.post.content} {carta}/>
            <button class="btn btn-primary my-4 w-full" on:click={() => savePost()}>Save</button>
